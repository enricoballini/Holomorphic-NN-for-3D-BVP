...
def generate_boundary_data():
    ...
    # define vectors a:
    bc_type[tags["-x"], 0] = np.array([1, 1, 1])
    bc_type[tags["x"], 0] = np.array([1, 1, 1])
    bc_type[tags["-y"], 0] = np.array([1, 1, 1])
    bc_type[tags["y"], 0] = np.array([1, 1, 1])
    bc_type[tags["-z"], 0] = np.array([1, 1, 1])
    bc_type[tags["z"], 0] = np.array([1, 1, 1])

    # define vectors b:
    bc_type[tags["-x"], 1] = np.array([0, 0, 0])
    bc_type[tags["x"], 1] = np.array([0, 0, 0])
    bc_type[tags["-y"], 1] = np.array([0, 0, 0])
    bc_type[tags["y"], 1] = np.array([0, 0, 0])
    bc_type[tags["-z"], 1] = np.array([0, 0, 0])
    bc_type[tags["z"], 1] = np.array([0, 0, 0])
