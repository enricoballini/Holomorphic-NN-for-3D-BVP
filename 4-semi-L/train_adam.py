...
def update_mins_maxes_all_over_epochs(
    ...
    mins_all, maxes_all = case_settings.compute_input_max_layer(
...
def train_adam(
    ...
    def make_update_fn(
        ...
        def update_adam(
            ...
            def grads_fn(p_list):
                loss, grads = utils_nn.compute_grads(
                    utils_nn.loss_l2,
                    p_list,
                    nn_list,
                    tm_list,
    ...
    for epoch in range(epochs):
        indices_batches = utils_nn.make_batch_idx(num_data, batch_size, seed=epoch)
        ...
        if epoch % 1000 == 0:
            utils_data_and_folders.save_params_list(params_list, 0)
            utils_data_and_folders.save_state_list(state_list, 0)
...
if __name__ == "__main__":
    ...
    der_displ_fcn = utils_mechanics.define_strain_from_displ()
    ...
    for idx_seed, seed in enumerate(seeds):
        ...
        params_list, state_list, adam, schedule, losses, current_epoch = (
            utils_nn.load_or_generate_params_list(architectures, key, tm_full_list)
        )
