...
def make_postprocess_domain_plot_boundary_and_inner():
    ...
    # Boundary nodes (actual coordinates)
    boundary_nodes = node_coords_3d[boundary_node_indices]

    # Inner node indices and nodes
    inner_node_indices = np.unique(inner_triangles.flatten())
...
def interpolate_node_values_to_face_centers(mesh, node_values, face_type="boundary"):
    ...
    Args:
        mesh: Mesh object with boundary_triangle_6node and/or inner_triangle_6node in face_data
        node_values: Array of shape (num_total_nodes, ...) - values at ALL nodes
        face_type: str, one of "boundary", "inner", or "all" (default: "boundary")

    Returns:
        face_values: Array of shape (num_faces, ...) - values at face centers
    """
...
def _batched_sigma(
    ...
    for start in range(0, n, batch_size):
        ...
        sigma_chunk = utils_mechanics.compute_sigma_tensor_from_displ(
            jacobian_field,
            tm_list,
            tm_full_list,
            params_list,
...
def _batched_displacement(
    ...
    for start in range(0, n, batch_size):
        chunk = points[start : start + batch_size]
        displ_chunk = utils_mechanics.compute_displacement(
            params_list,
            nn_list,
            tm_list,
            tm_full_list,
...
def _batched_div_sigma(
    ...
    def stress_tensor_single_pt(point):
    ...
    for start in range(0, n, batch_size):
        ...
        div_sigma_chunk = jax.vmap(lambda pt: divergence(stress_tensor_single_pt, pt))(
            chunk
        )

        results.append(np.array(div_sigma_chunk))
        print(
            f"  div sigma batch {start}–{min(start + batch_size, n) - 1} / {n - 1} done"
        )
    return np.concatenate(results, axis=0)
