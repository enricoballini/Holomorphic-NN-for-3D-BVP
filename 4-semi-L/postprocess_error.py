...
def compute_error(
    ...
    stress_exact_nodes, displ_exact_nodes = (
        utils_exact_solution.compute_exact_solution()
    )
    stress_exact = utils_postprocess.interpolate_node_values_to_tet_centroids(
        mesh, stress_exact_nodes
    )
    displ_exact = utils_postprocess.interpolate_node_values_to_tet_centroids(
        mesh, displ_exact_nodes
    )
    ...
    stress_exact_boundary = utils_postprocess.interpolate_node_values_to_face_centers(
        mesh, stress_exact_nodes, face_type="boundary"
    )
    traction_exact = utils_mechanics.compute_sigma_n(
    ...
    stress_nn_boundary = utils_postprocess._batched_sigma_from_displ(
        jacobian_field,
        tm_list,
        tm_full_list,
        params_list,
    ...
    displ_nn = utils_postprocess._batched_displacement(
        params_list,
        nn_list,
        tm_list,
        tm_full_list,
    ...
    traction_nn = utils_mechanics.compute_sigma_n(
    ...
    err_abs_distribution_displ = (
        utils_postprocess.interpolate_tet_centroid_values_to_nodes(
...
if __name__ == "__main__":
    ...
    # displacement:
    np.savetxt("./results/err_abs_displ", np.atleast_1d(err_abs_displ))
    np.savetxt(
        "./results/err_abs_distribution_displ",
        np.atleast_1d(err_abs_distribution_displ),
    )
    np.savetxt("./results/err_rel_displ", np.atleast_1d(err_rel_displ))
    np.savetxt(
    ...
    # stress:
    np.savetxt("./results/err_abs_stress", np.atleast_1d(err_abs_stress))
    np.savetxt(
        "./results/err_abs_distribution_stress",
        np.atleast_1d(err_abs_distribution_stress),
    )
    np.savetxt("./results/err_rel_stress", np.atleast_1d(err_rel_stress))
    np.savetxt(
    ...
    # tractions
    np.savetxt("./results/err_abs_traction", np.atleast_1d(err_abs_traction))
    np.savetxt(
        "./results/err_abs_distribution_traction",
        np.atleast_1d(err_abs_distribution_traction),
    )
    np.savetxt("./results/err_rel_traction", np.atleast_1d(err_rel_traction))
    np.savetxt(
        "./results/err_rel_distribution_traction",
