...
def print_min_max_weights():
    ...
    seed = 0
    params_list = utils_data_and_folders_pinn.load_params_list(seed)
...
def plot_mins_maxes_epochs(
    ...
    for ii, name in enumerate(mins_all_over_epochs):
        for idx_layer in range(mins_all_over_epochs[name].shape[1]):
            ax.plot(
                x_plot,
                mins_all_over_epochs[name][:, idx_layer][::points_to_skip],
            ...
            ax.plot(
                x_plot,
                maxes_all_over_epochs[name][:, idx_layer][::points_to_skip],
    ...
    ax.set_yscale("log")
    ax.set_ylim(bottom=10**-10, top=10**3)
    ...
    plt.savefig(
        "./results/mins_maxes_epochs_semi_L.pdf",
        dpi=150,
        bbox_inches="tight",
        pad_inches=0.2,
    )
    ...
    filename = "./results/mins_maxes_epochs_semi_L_legend.pdf"
    fig_legend.savefig(filename, bbox_inches="tight")
    plt.close(fig_legend)

    os.system(f"pdfcrop --margins '0 -0 -1500 0' {filename} {filename}")
    os.system(f"pdfcrop {filename} {filename}")


if __name__ == "__main__":

    with open("./results/mins_all_over_epochs_pinn.pkl", "rb") as fle:
        mins_all_over_epochs = pickle.load(fle)

    with open("./results/maxes_all_over_epochs_pinn.pkl", "rb") as fle:
