...
def common(params, reduction, point_coordinates, t):
    ...
    for layer in params[:-1]:
        ...
        z = utils_nn_pinn.activation_smooth(z @ layer["W"].T + layer["b"])
...
def compute_min_max_single_tm(params_list, nn_list, name, point_coordinates):
    # reduction = reductions[0]  ###

    _, mins, maxes = getattr(nn_list.der_0, name)(
        params_list[name], None, point_coordinates, None
    )
    return mins, maxes
...
if __name__ == "__main__":
    ...
    # default_architecture = np.array([3, 64, 64, 64, 64, 3]).astype(int)
    default_architecture = np.array([3, 2 * 64, 2 * 64, 2 * 64, 2 * 64, 3]).astype(int)
    default_split = np.array([1, 2, 2, 2, 2, 1])
