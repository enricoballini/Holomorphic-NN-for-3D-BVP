...
def compute_displacement(
    ...
    displ = nn_list.der_0.chi(
        params_list["chi"], reduction=None, point_coordinates=point_coordinates, t=0
    )
...
def compute_div_sigma(params_list, nn_list, nu, G, point_coordinates, jacobian_field):
    """ """
    tm_grid = jnp.array([0])
    tm_full_list = jnp.array([0])
    reductions = None
    nodes_int = jnp.array([0])
    weights_int = jnp.array([0])
    ...
    div_sigma = jax.vmap(lambda pt: divergence(stress_tensor, pt))(point_coordinates)
    return div_sigma[:, 0, :]
