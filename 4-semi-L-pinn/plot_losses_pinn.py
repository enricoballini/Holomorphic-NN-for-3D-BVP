...
def plot_losses_epochs(
    ...
    fig, ax = plt.subplots(figsize=(12, 4))  # matched to second code

    for idx_seed in idx_seeds:
        with open(f"./results/losses_seed{idx_seed}_pinn.pkl", "rb") as fle:
            losses = pickle.load(fle)

        x_plot = np.arange(0, losses["train"].shape[0] * test_every, test_every) + 1
        num_losses = losses["train"].shape[1]
        num_losses = 1  # matched to second code

        for idx_loss in range(num_losses):
            ax.plot(
                x_plot,
                losses["train"][:, idx_loss],
                color=colors_train[idx_loss],  # color by loss index, as in second code
                linestyle="--",
                linewidth=2,
                label="train",  # simplified label, as in second code
            )
            ax.plot(
                x_plot,
                losses["test"][:, idx_loss],
                color=colors_test[idx_loss],  # color by loss index, as in second code
                linestyle="-",
                linewidth=2,
                label="test",  # simplified label, as in second code
            )

    ax.set_yscale("log")
    ax.yaxis.set_major_locator(
        matplotlib.ticker.LogLocator(numticks=4)
    )  # matched to second code
    ax.grid(linestyle="--", linewidth=0.1, alpha=0.15, which="both")

    ax.set_ylabel(
        r"$\mathscr{L}$", fontsize=1.5 * fontsize
    )  # larger label, as in second code
    ax.set_xlabel("epoch")

    plt.legend()  # simple legend, as in second code

    plt.savefig(
        "./results/losses_epochs_pinn_mech.pdf",
        dpi=150,
        bbox_inches="tight",
        pad_inches=0.2,
    )

    # Dummy plot for train/test legend (Option 1, matched to second code)
    ...
    filename = "./results/losses_epochs_pinn_mech_legend.pdf"
...
colors_train = [[0.5, 0.5, 0.5], [0.8, 1, 0], [0.8, 0, 0.8]]  # matched to second code
colors_test = [[0, 0, 0], [0.5, 0.7, 0], [0.5, 0, 0.5]]  # matched to second code
plot_losses_epochs(
    idx_seeds,
    test_every,
    losses_names,
