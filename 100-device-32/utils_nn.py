...
@jax.jit
def compute_loss_val(bc_type, V_g, nabla_V_g_n, bc_vals, weights_loss, epoch=None):
    diff = bc_type[:, 0] * V_g + bc_type[:, 1] * nabla_V_g_n - bc_vals

    return (jnp.mean(weights_loss * jnp.conj(diff) * diff)).astype(jnp.complex64)
...
def loss_l2(
    ...
    loss_val = compute_loss_val(
        bc_type, V_g, nabla_V_g_n, bc_vals, weights_loss, epoch=epoch
    )
